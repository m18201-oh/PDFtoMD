...
- 입력 폴더: `C:\Code\PDFtoMD\watch`
- 결과 파일은 `C:\Code\PDFtoMD\done` 폴더에 생성됩니다.

즉, 폴더 역할은 이렇게 나뉩니다.

- `watch`: PDF를 넣는 곳
- `done`: 변환이 끝난 결과를 확인하는 곳

## 3. 평소 사용하는 방법

### 3.1 자동 감시 상태로 사용

PC 로그인 시 자동 실행되도록 등록해 두었다면, `watch` 폴더에 PDF만 넣으면 됩니다.
...
- `done` 폴더에 같은 이름의 `.md` 파일이 생깁니다.
- 원본 PDF도 `done` 폴더로 이동합니다.
...
- 실행 로그: `C:\Code\PDFtoMD\logs\run.log`
- 오류 로그: `C:\Code\PDFtoMD\logs\error.log`
...
- 입력: `C:\Code\PDFtoMD\watch`
- 최종 결과 확인: `C:\Code\PDFtoMD\done`
- 완료 원본: `C:\Code\PDFtoMD\done`
- 실패 격리: `C:\Code\PDFtoMD\Rejected`
- 진행 중 작업 흔적: `C:\Code\PDFtoMD\workspace`
- 로그: `C:\Code\PDFtoMD\logs`

## 9. 가장 중요한 한 줄 판단 기준

`done`에 결과 `.md`, 원본 PDF, 작업 브리핑 `.md`가 함께 보이면 성공입니다.

그렇지 않으면 `Rejected` 또는 `logs`를 먼저 보면 됩니다.
