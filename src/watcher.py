...
main.py가 01_watch_inbox/ 전체를 직접 스캔하여 배치 처리.
"""
...
from src.config import WATCH_DIR, PROJECT_ROOT
from src.modules.runtime_state import lock_is_active
...
def check_pending_pdfs(reason: str):
    ...
    log.info("01_watch_inbox에 처리 대기 PDF %d개 존재", len(pending_pdfs))
    trigger_main(reason)


class PDFHandler(FileSystemEventHandler):
    """01_watch_inbox 폴더에 .pdf 파일이 생성되면 main.py를 기동한다."""
...
def main():
    ...
    check_pending_pdfs("01_watch_inbox 기존 PDF 감지")

    try:
        while True:
            time.sleep(POLL_INTERVAL_SECONDS)
            check_pending_pdfs("01_watch_inbox 폴더 폴링 감지")
