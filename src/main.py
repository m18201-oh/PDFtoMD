...
동작 흐름:
  ...
  4. 01_watch_inbox/ 폴더에서 .pdf 파일 전체 목록 수집
...
# ── 모듈 임포트 ───────────────────────────────────────────
from src.modules.m1_ingest import ingest
from src.modules.m1_5_validator import validate
from src.modules.m2_split import split_pdf
from src.modules.m3_deliver import deliver
from src.modules.m4_collect import collect
from src.modules.m5_merge import merge
from src.modules.m6_deploy import deploy
from src.modules.rollback import rollback
from src.modules.reporter import send_complete, send_error, send_progress
from src.modules.runtime_state import acquire_lock, lock_is_active, release_lock


def _job_status(job_id: str | None) -> str | None:
    if not job_id:
        return None
...
def process_single_pdf(pdf_path: Path) -> dict:
    ...
    try:
        ...
        if validated_pdf is None:
            ...
            # 01_watch_inbox/의 원본도 03_rejected로 이동하여 무한 루프 방지
        ...
        logger.info("[M6] 배치 완료: %s → 04_done/", merged_md.name)
        ...
        if job_id:
            ...
            preserve_state = current_status in {"split", "delivered", "collected", "merged"}
            rollback(
                job_id,
                release_lock=False,
                preserve_converter_state=preserve_state,
                preserve_workspace=preserve_state,
            )
        try:
...
def resume_incomplete_jobs() -> list[dict]:
    from src.config import WORKSPACE_DIR, WATCH_DIR
    results = []
    ...
    for job_dir in sorted(WORKSPACE_DIR.iterdir()):
        ...
        try:
            ...
            if job_id:
                preserve_state = status in {"split", "delivered", "collected", "merged"}
                rollback(
                    job_id,
                    release_lock=False,
                    preserve_converter_state=preserve_state,
...
def main():
    ...
    try:
        ...
        # 3) 01_watch_inbox/ 전체 PDF 스캔 (방식B)
