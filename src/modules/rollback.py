...
def rollback(
    ...
    preserve_converter_state: bool = False,
    preserve_workspace: bool = False,
):
    """
    중단 상황에서 잔존 파일 정리.
    release_lock=True 일 때만 .lock 삭제 (main.py의 finally에서 직접 삭제하는 경우 False).
    preserve_converter_state=True 이면 converter input/output을 유지한다.
    preserve_workspace=True 이면 해당 job workspace를 유지한다.
    """
    logger.warning("[ROLLBACK] 롤백 시작 — job_id=%s", job_id)

    try:
        # ① docuConverter01/input/ 정리
        if preserve_converter_state:
            logger.info("[ROLLBACK] ① converter/input/ 유지")
        else:
            try:
        ...
        if preserve_converter_state:
            logger.info("[ROLLBACK] ② converter/output/ 유지")
        else:
            try:
        ...
        # ③ 02_workspace/{job_id}/ 삭제
        if preserve_workspace:
            logger.info("[ROLLBACK] ③ 02_workspace/%s/ 유지", job_id)
        else:
            try:
                job_dir = WORKSPACE_DIR / job_id
                if job_dir.exists():
                    shutil.rmtree(str(job_dir))
                    logger.info("[ROLLBACK] ③ 02_workspace/%s/ 삭제 완료", job_id)
            except Exception as e:
                logger.error("[ROLLBACK] ③ 02_workspace/%s/ 삭제 실패: %s", job_id, e)
