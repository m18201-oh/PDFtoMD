...
- 최신 마무리 결과:
  - `C:\Code\PDFtoMD\04_done`에 2026 문서 최종 `.md`, 원본 PDF, `_briefing.md` 생성 완료
  - `C:\Code\docuConverter01\input`은 `0개`
  - `C:\Code\docuConverter01\output`은 `24페이지 변환 결과` 기준으로 회수 완료

## 2. 현재 운영 기준

1. Windows 로그인 시 `HKCU Run`에 등록된 watcher가 자동 시작
2. watcher는 새 PDF 이벤트를 즉시 감지
3. 새 파일 이벤트가 없더라도 `30분` 주기로 `01_watch_inbox`를 재확인
4. 원본 PDF가 남아 있고 `.lock`이 활성 상태가 아니면 `main.py` 트리거
